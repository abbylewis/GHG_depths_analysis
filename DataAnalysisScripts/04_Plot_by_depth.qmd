---
title: "Concentrations and solubility by depth"
format: html
editor: visual
---
...
full <- data %>%
  ...
  mutate(Depth_m = ifelse(Depth_m > MaximumDepth_m,
                          MaximumDepth_m,
                          Depth_m)) 
...
points <- full %>%
  ...
  ggh4x::facetted_pos_scales(y = list(
    scale_y_continuous(
      trans = scales::pseudo_log_trans(0.001, 100000),
      labels = scales::label_comma(drop0trailing = T),
      breaks = c(0, 0.1, 10, 1000)
    ),
    scale_y_continuous(
      trans = scales::pseudo_log_trans(0.1, 1000000),
      labels = scales::label_comma(drop0trailing = T),
...
epi_hypo <- epi_hypo_data %>%
  ...
  ggh4x::facetted_pos_scales(y = list(
    ...
    scale_y_continuous(
      trans = scales::pseudo_log_trans(1, 1000000),
      labels = scales::label_comma(drop0trailing = T),
  ...
  labs(
    y = expression("Surface concentration (µM)"),
...
comb <- ggpubr::ggarrange(points, epi_hypo,
  nrow = 2,
  common.legend = T,
...
sat_fig <- sat %>%
  ...
  scale_color_gradient(
    low = "#2255dd", high = "#dd2255",
    name = "Temperature (ºC)"
  ) +
  ggh4x::facet_grid2(Layer ~ Gas,
    scales = "free",
...
sat %>%
  ...
  geom_text(
    aes(
      x = 0.2, y = ifelse(Gas == "CO[2]", 600000, 80000000),
      label = paste0(
        "Median = ",
...
sat %>%
  filter(Gas == "CH[4]", pct < 100, !Conc == 0)

nrow(sat %>% filter(Gas == "CH[4]", pct < 100, !Conc == 0)) / nrow(sat %>% filter(Gas == "CH[4]", pct < 100)) * 100
```
...
jpeg("../Figures/Saturation departure.png",
  res = 300, width = 5, height = 3,
  units = "in"
)
sats %>%
  mutate(Layer = factor(Layer,
  ...
  scale_color_gradientn(
    colors = c("red", "pink", "blue", "darkblue"),
    values = c(0, .1, .6, 1),
    name = "DO (mg/L)"
  ) +
  theme(
