...
# Load data from EDI
data <- read_csv("https://pasta.lternet.edu/package/data/eml/edi/2220/5/ca2482ef3c43f053ae13cdf2bf5ba7a8?key=n9dYnNE5ReoA53SyzOCNgcPzLZA") %>%
  filter(Focal_site) # Not cleaned
cleaned_data <- read_csv("https://pasta.lternet.edu/package/data/eml/edi/2220/5/8553f79eeea9a6f722fc50d733ca9a74?key=n9dYnNE5ReoA53SyzOCNgcPzLZA") %>%
  filter(Focal_site) # Cleaned
temp <- read_csv("https://pasta.lternet.edu/package/data/eml/edi/2220/5/a101f4f31d3b2aa3dd81490892bd0b15?key=n9dYnNE5ReoA53SyzOCNgcPzLZA") %>%
  filter(Focal_site)
...
a <- layer_sum %>% filter(Layer == "surf") %>% mutate(ratio = Depth_m/MaximumDepth_m) %>%
  ...
  labs(x = "Perecent of max depth", y = "Count of 'surface' meaurments") +
  theme_bw()
...
b <- layer_sum %>% filter(Layer == "bot") %>% mutate(ratio = Depth_m/MaximumDepth_m) %>%
  ggplot(aes(x = ratio*100)) +
  geom_histogram(binwidth = 2) +
  labs(x = "Perecent of max depth", y = "Count of 'bottom' meaurments") +
...
true_max_depths <- layer_sum_not_mean %>% left_join(metadata) %>%
  group_by(LakeID) %>% summarize(
        true_max_depth = ifelse(max(Depth_m, na.rm = T) > first(MaximumDepth_m), 
                                max(Depth_m, na.rm = T), first(MaximumDepth_m))
  )
