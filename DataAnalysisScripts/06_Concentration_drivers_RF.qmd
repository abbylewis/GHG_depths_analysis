...
layer_plot_data <- layer_sum %>%
  ...
  filter(!is.na(value)) %>%
  mutate(feature = ifelse(value == max(value), #looking at the drivers of the lake-wise max
                          T,
                          F)) %>%
...
all_consistent <- all %>%
  select(all_of(c("value", "LakeID", vars))) %>%
  drop_na() %>%
  group_by(LakeID) %>%
  filter(n() == 4) #surface and bot co2 and ch4
...
colors <- c("#40476D", "#0C7C59", "#1098F7", "grey70")
names(colors) <- c("Log surface area", "Trophic status", "Log max depth")
