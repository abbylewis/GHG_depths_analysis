...
source("R/spatial_comp_permutation.R")
...
layer_plot_data <- layer_sum %>%
  ...
  temp_dif = ifelse(length(Temp_C) == 2,
    Temp_C[Layer == "surf"] -
      Temp_C[Layer == "bot"],
    NA
  )) %>%
...
layer_plot_data %>%
  ungroup() %>%
  select(Temp_C, Source, Date, Layer, LakeID, temp_dif, name) %>%
  pivot_wider(names_from = Layer, values_from = Temp_C) %>%
  filter(temp_dif < -3,
         surf > 4)
...
all <- layer_plot_data %>%
  ...
  group_by(name) %>%
  mutate(
    value = ifelse(value <= 0, min(value[value > 0], na.rm = TRUE) / 2, value),
    value = log(value))
...
for_cor <- all %>%
  ...
  pivot_wider(
    names_from = Layer,
    values_from = any_of(c("value", "DO_mgL", "Temp_C"))
  ) %>%
  pivot_wider(
    names_from = name,
    values_from = c(value_surf, value_bot)
  ) %>%
  select(
    where(is.numeric),
    -Longitude,
...
dens_plots <- for_cor %>%
  ...
  mutate(name = factor(name,
    levels = c(
      "DO_mgL_bot", "DO_mgL_surf", "SurfaceArea_km2", "MaximumDepth_m","Osgood",
      "TP_ugL_mean", "Temp_C_bot", "Temp_C_surf","temp_dif",
      "Absolute_latitude",
      "value_surf_CH[4]", "value_bot_CH[4]", "value_surf_CO[2]",
      "value_bot_CO[2]"
    ),
    labels = c(
      "Bottom~DO~(mg~L^-1)", "Surface~DO~(mg~L^-1)", "SA~(log~km^2)","Max.~depth~(log~m)",
      "Osgood~index~(log)",
      "TP~(log~µg~L^-1)", "Bottom~temp~(ºC)", "Surface~temp~(ºC)","Temp.~diff.~(ºC)",
      "Abs.~latitude",
...
ggsave(s, dpi = 300, filename = "../Figures/Model comparison - rmse comb imp - permutation.jpeg", width = 6, height = 5.5)
```
