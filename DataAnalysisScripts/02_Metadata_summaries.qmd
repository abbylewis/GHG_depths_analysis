...
label_grat_y <- grat_wintri %>% st_drop_geometry %>% filter(degree != 89.9) %>%
  ...
  select(degree_label, x_start, y_start) %>% 
  rename(long = x_start, lat = y_start) %>% st_as_sf(coords = c("long", "lat"), crs = 4326) %>%
  st_transform_proj(crs = crs_wintri)
...
map <- ggplot(data = world_wintri) +
  ...
  geom_sf(data = grat_wintri, color = "black", size = 0.05, alpha = 0.7, 
          linetype = "dotted") + 
