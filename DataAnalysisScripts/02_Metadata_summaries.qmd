...
#hydrolakes
#hylak_shp <- read_sf(dsn = "../../GasHype/RawData/HydroLAKES/HydroLAKES_polys_v10.shp", layer = "HydroLAKES_polys_v10") %>% st_drop_geometry() %>%
hylak_shp <- read_sf(dsn = "../CompiledData/HydroLAKES_points_v10.dbf") %>% st_drop_geometry() %>%
  select(Hylak_id, Lake_area, Depth_avg, Pour_long, Pour_lat)
```
...
depth <- metadata %>%
  ...
  #geom_density(data = hylak_shp, aes(x = Depth_avg), color = "gray",
   #            fill = "gray90", alpha = .8) +
  geom_density() +
  scale_x_log10(
...
area <- metadata %>%
  ...
  #geom_density(data = hylak_shp, aes(x = Lake_area), color = "gray",
   #            fill = "gray90", alpha = .8) +
  geom_density() +
  scale_x_log10(
...
lat <- metadata %>%
  ...
  ggplot(aes(y = lat)) +
  geom_density(data = hylak_shp, aes(y = Pour_lat), color = "gray",
               fill = "gray90", alpha = .8) +
  geom_density() +
  scale_x_continuous(limits = c(0, NA), expand = c(0, 0), 
                     breaks = c(0, 0.02, 0.04)) +
  scale_y_continuous(breaks = seq(-70, 70, 20),
                     labels = parse(text = c('"70"*degree*S', '"50"*degree*S', '"30"*degree*S',
                                '"10"*degree*S', '"10"*degree*N', '"30"*degree*N',
                                '"50"*degree*N', '"70"*degree*N'))) +
  theme_bw() +
  theme(
    ...
    axis.text = element_text(size = 2.2*ggplot2::.pt),
...
map <- ggplot(data = world_wintri) +
  ...
  #  nudge_x = c(1400000), size = 2.2
  #) +
  geom_sf_text(
    data = label_grat_x, aes(label = degree_label), parse = T,
    nudge_y = c(400000), size = 2.2, color = "grey30"
  ) +
...
# full map with latitude
map2 <- map + theme(plot.margin = unit(c(0.5, 50, 1.5, 30), "pt"))
full <- map2 %>%
  ggdraw() +
  draw_plot(usa,
    x = -0.05,
  ...
  draw_plot_label("a2", x = 0.83, y = 0.94, size = 10)+
  draw_plot_label("a1", x = -0.008, y = 0.94, size = 10)
...
comb <- ggpubr::ggarrange(full,
  ...
  nrow = 2, heights = c(2, 1), labels = c("a", "b"), label.y = c(1, 1.12)
)
ggsave(
...
sum <- metadata %>%
  ...
  group_by(LakeID) %>%
  mutate(
    StratificationRegime = ifelse(length(unique(StratificationRegime)) > 1,
      NA,
