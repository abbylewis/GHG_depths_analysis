...
formatted_data <- layer_sum %>%
  ...
  summarize(
    CH4_umolL = median(CH4_umolL, na.rm = T),
...
tot <- formatted_data %>%
  mutate(
    total_umolL = CH4_umolL + CO2_umolL,
    ratio_ch4_co2 = CH4_umolL / CO2_umolL
  ) %>%
  filter(
    !is.na(Layer), !is.na(ratio_ch4_co2), !Oxygen > 15,
    Layer == "bot"
  ) %>%
  ...
  scale_color_gradientn(
    name = expression("Ratio of " ~ CH[4] * ":" * "CO"[2]),
    transform = "log10",
...
tot_full <- formatted_data %>%
  ...
  filter(
    !is.na(Layer), !is.na(ratio_ch4_co2),
    Layer == "bot"
  ) %>%
  mutate(ratio_ch4_co2 = ifelse(ratio_ch4_co2 == 0, 0.0001,
    ratio_ch4_co2
...
medians <- formatted_data %>%
  ...
  mutate(
    label = paste0("Median: ", round(median), " ~mu", "*M"),
    y = c(0.59, 0.5)
  )
...
ggsave(tot_full + theme(legend.position = "bottom"),
  dpi = 300,
