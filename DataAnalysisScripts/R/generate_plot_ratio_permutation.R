...
generate_plot_ratio_permutation <- function(all_co2, 
  ...
  surf_name <- paste0("CO[2]*' ('*italic(n)*' = ", length(unique(for_rf_surf$LakeID)),
                      "; mean '*R^2*' = ", round(surf_r2, 2), ")'")
  bot_name <- paste0("CH[4]*' ('*italic(n)*' = ", length(unique(for_rf_bot$LakeID)),
  ...
  importance <- ImpData %>%
    ...
    geom_col()+
    geom_errorbar(aes(xmin = `%IncMSE` - sd,
                      xmax = `%IncMSE` + sd),
                  width = 0.1)+
  ...
  part <- partials_sorted %>%
    ...
    summarize(sd = sd(y, na.rm = T),
              y = mean(y, na.rm = T)) %>%
    ggplot(aes(x = x))+
    {if(pSat)geom_hline(yintercept=100, lty = "11")}+
    geom_ribbon(aes(ymax = y + sd, 
                    ymin = y - sd, 
                    fill = var),
                alpha = 0.5) +
    ...
    theme(
      ...
      strip.text.x = element_text(margin = unit(c(0,0,0,0), units = "cm"),
                                  size = 8),
    ...
    ylab(substitute("Predicted bot:surf ratio"))+
    facetted_pos_scales(
      x = list(
