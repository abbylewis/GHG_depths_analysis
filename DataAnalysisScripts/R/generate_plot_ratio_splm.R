...
generate_plot_ratio_splm <- function(all_co2, all_ch4, vars, var_names, log_vars, pSat = F){
  ...
  var_names_imp = sub("\n", " ", var_names)
  imp_levels_used <- rev(recode(c(vars_reg, "LakeID"), !!!var_names_imp))
  names(colors) <- var_names_imp[vars_reg]
  
  surf_name <- paste0("CO<sub>2</sub> (", nrow(for_rf_surf), 
                      " measurements, ", length(unique(for_rf_surf$LakeID)),
                      " lakes; R<sup>2</sup> = ", round(surf_r2, 2), ")")
  bot_name <- paste0("CH<sub>4</sub> (", nrow(for_rf_bot), 
  ...
  importance <- ImpData %>%
    mutate(Var.Names = recode(Var.Names, !!!var_names_imp),
           ...
           Layer = case_match(Layer,
                              "CO[2]"~surf_name,
                              "CH[4]"~bot_name),
           Layer = factor(Layer, levels = c(bot_name, surf_name))) %>%
    ...
    theme(strip.text = element_textbox_simple(
      size = 9,
      halign = 0.5, # Center alignment
      valign = 0.5,
      padding = ggplot2::margin(5, 5, 5, 5), # Add some padding
      margin = ggplot2::margin(5, 5, 5, 5) # Add some margin
    ))
  ...
  for_rug <- for_rug_surf %>% mutate(Layer = "CO[2]") %>%
    bind_rows(for_rug_bot %>% mutate(Layer = "CH[4]")) %>%
    mutate(Layer = factor(Layer, levels = c("CH[4]","CO[2]"))) %>%
    mutate(x = ifelse(var %in% recode(log_vars, !!!var_names_part),
                      exp(x),
                      x))
  
  #Plot partials
  partials_sorted <- partials_df_surf %>% mutate(Layer = "CO[2]") %>%
    bind_rows(partials_df_bot %>% mutate(Layer = "CH[4]")) %>%
    mutate(var = recode(var, !!!var_names_part),
           var = factor(var, levels = levels_used),
           y = exp(y),
           Layer = factor(Layer, levels = c("CH[4]", "CO[2]")),
