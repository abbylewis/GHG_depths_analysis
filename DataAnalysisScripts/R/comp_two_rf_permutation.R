...
comp_two_rf_permutation <- function(all,
  ...
  imp <- ImpData %>%
    ...
    mutate(fct_order = case_when(
      gas == "CH[4]" & Layer == "Surface" ~ 1,
  ...
  eval_sum <- eval %>%
    pivot_longer(c(rmse_simple, rmse_complex),
      names_prefix = "rmse_",
      values_to = "rmse"
    ) %>%
...
permutation_rf_comp <- function(rep_n, for_rf, vars_simple,
  ...
  rf_simple <- randomForest(value ~ .,
    data = for_rf_simple,
    proximity = TRUE, importance = T
  )

  rf_complex <- randomForest(value ~ .,
    data = for_rf_complex,
    proximity = TRUE, importance = T
  )
  ...
  preds <- data.frame(
    rf_simple_pred = predict(rf_simple),
  ...
  out <- preds %>%
    summarize(
      rmse_simple = Metrics::rmse(obs, rf_simple_pred),
