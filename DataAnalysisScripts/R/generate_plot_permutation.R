...
generate_plot_permutation <- function(all, vars, var_names, gas_name, log_vars, 
  ...
  surf_name <- paste0("'Surface ('*italic(n)*' = ", length(unique(for_rf_surf$LakeID)),
                      "; mean '*R^2*' = ", round(surf_r2, 2), ")'")
  bot_name <- paste0("'Bottom ('*italic(n)*' = ", length(unique(for_rf_bot$LakeID)),
  ...
  part <- partials_sorted %>%
    ...
    facetted_pos_scales(
      x = list(
        ...
        var == "Buoy~freq~(1/s)" ~ scale_x_continuous(trans = scales::pseudo_log_trans(0.0001,10), 
                                                      labels = scales::label_comma(drop0trailing = T),
